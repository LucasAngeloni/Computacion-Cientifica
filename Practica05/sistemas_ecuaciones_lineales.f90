module sistemas_ecuaciones_lineales
    ...
    subroutine permutacion_filas(A_amp,fila)
        ...
        if ( fila == n_filas ) then
            write(*,*) "Pivote igual a 0 y no se puede permutar las filas"
            stop           
        end if
    ...
    subroutine eliminacion_gauss_jordan_msist(A,B,x)
        implicit none
        !Declaración parametros de entrada y salida
        real(rkind8),intent(in) :: A(:,:),B(:,:)
        real(rkind8),intent(out) :: x(:,:)
        ...
        deallocate(A_amp)
    end subroutine eliminacion_gauss_jordan_msist
